...
| additional_environment_variables | Additional environment variables to set on the forwarder Lambda function. Merged after all built-in variables, so these take precedence on conflict.              | `map(string)`  | `{}`                |
