...
| additional_environment_variables | Additional environment variables to set on the forwarder Lambda. Merged last, so these take precedence over built-in variables on conflict.              | `map(string)`  | `{}`                |
