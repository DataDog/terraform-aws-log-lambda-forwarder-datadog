...
variable "dd_api_key_secret_arn" {
  type        = string
  default     = "arn:aws:secretsmanager:REPLACEME"
  ...
  validation {
    condition     = can(regex("^arn:.*:secretsmanager:.*", var.dd_api_key_secret_arn))
...
variable "dd_api_key_ssm_parameter_name" {
  ...
  validation {
    condition     = var.dd_api_key_ssm_parameter_name == "" || can(regex("^/[a-zA-Z0-9/_.-]*$", var.dd_api_key_ssm_parameter_name))
...
variable "reserved_concurrency" {
  type        = string
  default     = ""
  description = "Reserved concurrency for the Datadog Forwarder Lambda function. If empty, use unreserved account concurrency. We recommend using at least 10 reserved concurrency, but default to 0 as you may need to increase your limits for this. If using unreserved account concurrency you may limit other lambda functions in your environment."

  validation {
    condition     = var.reserved_concurrency == "" || can(tonumber(var.reserved_concurrency))
    error_message = "reserved_concurrency must be a valid integer number or empty string."
  }
}
...
variable "dd_compression_level" {
  ...
  validation {
    condition     = var.dd_compression_level >= 0 && var.dd_compression_level <= 9
    error_message = "dd_compression_level must be between 0 and 9."
  }
}
...
variable "additional_target_lambda_arns" {
  type        = list(string)
  default     = []
  description = "List of lambda ARNs that get invoked asynchronously with the same input event"
}
...
variable "dd_log_level" {
  ...
  validation {
    condition     = contains(["DEBUG", "INFO", "WARN", "ERROR", "CRITICAL"], var.dd_log_level)
